...
template <typename T>
static inline T *alloc_cuda_ptr(std::size_t _size) {
  T *p = nullptr;
...
template <typename T>
static inline void free_cuda_ptr(T *p) {
...
class cuda_ptr {
 ...
 public:
  ...
  cuda_ptr(std::size_t _size)
      : __size(_size), holder(alloc_cuda_ptr<T>(_size), free_cuda_ptr<T>) {
    if (not bool(holder)) {
